...
int HandleInstruction(const string& instruction, vector<Registery>& registeries, vector<string>& instructionVector, const int instructionIndex, string& outputString)
{
	...
	else if (instruction.find("out") != string::npos)
	{
		outputString += to_string(registeries[instruction.back() - 'a'].value);
	}
...
void main()
{
	vector<Registery> regs{ { 'a', 0 },{ 'b', 0 },{ 'c', 0 },{ 'd', 0 } };
	vector<string> instructions;
	ifstream input("./Inputs/Input25.txt");
	if (input.is_open())
	{
		string line;
	...
	for (int j = 0; j < 10000; ++j)
	{
		...
		{
			i += HandleInstruction(instructions[i], regs, instructions, i, output);
			++instructionsHandled;
			if (instructionsHandled > 50000000)
			{
				break;
			}
		...
		cout << "initial value of registery a: " << j << endl;
		cout << output << endl;
		if (foundMatch)
			break;
	}
